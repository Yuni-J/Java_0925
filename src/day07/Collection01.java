...
public class Collection01 {
	...
	public static void main(String[] args) {
		 ...
		 * - value 가 규제된다 (타입을 정하고 그 타입만 사용가능)
		 * - index 는 없고, key 가 index 역할을 함.
		 * - key 는 중복불가능 / value 중복가능.
		 * - key 가 중복되면 value 는 덮어써짐.
		 * - 2개의 값이 하나의 자료로 묶어야 할 때 사용
		 * - ex) 아이디/패스워드, 제품명/가격, 학생명/점수
		 * 
		 * 배열은 기본자료형
		 * int[] arr, double[] arr, String[] arr, Student[] arr 다 가능.
		 * 
		 * Collection 은 기본자료형은 불가능함. 클래스 자료형만 가능.
		...
		int num = 10;
//		Integer boxNum = new Integer(num);  // 잘쓰지않는다.
...
//		System.out.println(a);
		...
		// - set(index, valuse) : index번지의 값을 value로 번경
		...
		// - contain(Object) : list에 값이 있는지 검사 true / false로 리턴
		System.out.println(strList.contains("아침"));
		System.out.println(strList.contains("밤"));
		...
		list.sort(new Comparator<Integer>() {
			...
			public int compare(Integer o1, Integer o2) {
				// 결과가 -면 앞으로 보내고, +면 뒤로 보내는 역할 (o1이 기준)
