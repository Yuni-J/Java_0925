 ...
 * - 각각의 객체는 메서지를 주고 받으면서 데이터를 처리
 * - 메서지를 받을 경우(매개변수를 통해) / 메시지를 주는 경우(return)
 * - 추상화, 캡슐화(정보은닉), 상속, 다형성
 * 
 ...
 * - 맴버변수(private)를 숨기는 형태, 메서드로 접근
 * - 맴버변수와 메서드를 하나로 묶어 처리하는 형태
 ...
 * - 다형성을 잘 활용하면 유연하고, 확장성있는, 유지보수가 편리한 프로그램을 만들 수 있
 * */
...
//		ex01.moveAnimal(dog);  
		...
		//Human 클래스 객체에 Animal을 상속받은 Human 클래스 객체를 다운 케스팅한 예시
		Human h = (Human)hAnimal;
		h.reakBook();
		
//		Human h1 = (Human)tAnimal;
//		h1.reakBook();  //casting 오류 : 다운 케스팅의 형이 맞지않아서 error
...
//			((Human) downCastingTest).reakBook();  //한번에 할려면
	...
	public void moveAnimal(Animal animal) {  //public void moveAnimal(Human h, Tiger t, Eagle e) { 각각 불가능 if로 불어들이기 힘들어서...
		animal.move();
		
		if(animal instanceof Human) {
			((Human) animal).reakBook();
